...
import { Movie, Movie2, Seats } from '../../constants/models/Movies'
...
const Seats = () => {
  ...
  const movie:Movie2 = movies?.find((mov:any) => mov.id === parseInt(id));
        ...
        <Link href={{ pathname: '/payment', query: { movieId: movie?.id, seatDetails: JSON.stringify(seatDetails) } }}>
          <div className={styles.paymentButtonContainer}>
            <Button variant="contained" href="#contained-buttons" className={styles.paymentButton} >
              Оплатить ₽{selectedSeats.length * (movie?.ticketCost || 0)}
            </Button>
          </div>
        </Link>
      )
